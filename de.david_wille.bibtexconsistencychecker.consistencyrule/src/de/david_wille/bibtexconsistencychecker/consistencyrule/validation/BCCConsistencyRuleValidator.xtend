...
class BCCConsistencyRuleValidator extends AbstractBCCConsistencyRuleValidator {
	...
	protected def checkExceptForBodySpecifiesSameSelectorAsAppliesToBody(BCCAbstractSelectorBody exceptForEntryBody, BCCAbstractSelectorBody appliesToEntryBody) {
		var List<BCCAbstractSelector> exceptForSelectors = identifySelectors(exceptForEntryBody)
		...
		for (BCCAbstractSelector exceptForSelector : exceptForSelectors) {
			if (sameSelectorContained(appliesToSelectors, exceptForSelector)) {
				if (exceptForEntryBody instanceof BCCMultiEntrySelectorBody) {
				...
				else if (exceptForEntryBody instanceof BCCSingleEntrySelectorBody) {
